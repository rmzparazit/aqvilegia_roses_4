...
class Pattern:
    def __init__(self):
        self.trash_params = ['ok', 'отмена', 'имя', 'заказ', 'телефон', 'отправить', 'открытка', 'скидка']
        self.seo_stop_words = [
            ' в москве', ' с доставкой', ' купить', ' заказать',
            ' недорого', ' дешево', ' цена', ' свежие розы'
        ]

    def match_score(self, url: str, html_content: str, raw_product) -> float:
        score = 0.0
        text_lower = raw_product.h1_title.lower() + html_content.lower()[:500]
        if "aqvilegia.ru" in url or any(w in text_lower for w in ["букет", "роза", "пион", "тюльпан"]):
            score += 0.9
        return min(score, 1.0)
        
    def fix_price(self, raw_product, html_content: str):
        soup = BeautifulSoup(html_content, 'lxml')
        price_el = soup.select_one('.product-card__actual-price')
        if price_el:
            raw_product.price_raw = re.sub(r'[^\d]', '', price_el.get_text())
        oldprice_el = soup.select_one('.product-card__old-price')
        if oldprice_el:
            raw_product.oldprice_raw = re.sub(r'[^\d]', '', oldprice_el.get_text())
        return raw_product

    def clean_title(self, title: str) -> str:
        lower_title = title.lower()
        for sw in self.seo_stop_words:
            idx = title.lower().find(sw)
            if idx != -1:
                title = title[:idx]
        return title.strip()

    def get_dynamic_category(self, raw_product) -> str:
        title = raw_product.h1_title.lower()
        count = ""
        count_match = re.search(r'\b(\d{2,3})\b', title)
        if count_match: count = f" ({count_match.group(1)} шт.)"
        
        if "пионовид" in title: return f"Пионовидные розы{count}"
        if "пион" in title: return f"Пионы{count}"
        if "тюльпан" in title: return f"Тюльпаны{count}"
        
        color = ""
        if "красн" in title: color = "Красные "
        elif "бел" in title: color = "Белые "
        elif "розов" in title: color = "Розовые "
        elif "желт" in title or "жёлт" in title: color = "Желтые "
        elif "оранжев" in title: color = "Оранжевые "
        elif "малинов" in title: color = "Малиновые "
        elif "кремов" in title: color = "Кремовые "
        
        if "роз" in title: return f"{color}розы{count}".strip()
        return "Авторские букеты"

    def get_category_description(self, category_name: str) -> str:
        # Коротко и по делу. Лимит 81 символ.
        return f"{category_name.capitalize()}: свежие цветы, авторская сборка и доставка"

    def generate_offer_collection_desc(self, title: str) -> str:
        # Описание для коллекций-офферов. Лимит 81 символ.
        short_title = title[:40] 
        return f"{short_title}. Доставка от 20 мин. Фото до отправки"
        
    def generate_offer_description(self, raw_product) -> str:
        # 💡 ИСПОЛЬЗУЕМ ТЕГ {price}. Лимит 175 символов.
        title = self.clean_title(raw_product.h1_title)
        return f"Свежий {title.lower()} за {{price}} руб. Авторская сборка от флористов. Бережная и быстрая доставка по Москве и МО!"

    def filter_specs(self, specs: dict) -> dict:
        clean_specs = {}
        for k, v in specs.items():
            k_lower = str(k).lower().strip()
            v_lower = str(v).lower().strip()
            if not any(trash in k_lower for trash in self.trash_params) and not any(trash in v_lower for trash in self.trash_params):
                clean_specs[k] = v
        return clean_specs

    def filter_labels(self, labels: list, title: str) -> list:
        # 💡 УМНАЯ РАЗБИВКА НА ТЕГИ ДЛЯ ФИЛЬТРОВ ЯНДЕКС ДИРЕКТА
        valid = []
        title_lower = title.lower()
        
        for lbl in labels:
            if lbl.lower() in ['в корзине', 'в шляпной коробке']:
                valid.append(lbl)
                
        if 'роз' in title_lower: valid.append('Розы')
        if 'пион' in title_lower: valid.append('Пионы')
        if 'тюльпан' in title_lower: valid.append('Тюльпаны')
        if 'хризантем' in title_lower: valid.append('Хризантемы')
        
        colors = {
            'красн': 'Красные', 'бел': 'Белые', 'розов': 'Розовые',
            'желт': 'Желтые', 'жёлт': 'Желтые', 'оранжев': 'Оранжевые',
            'малинов': 'Малиновые', 'кремов': 'Кремовые'
        }
        ...
        match = re.search(r'\b(\d{2,3})\b', title_lower)
        if match:
            valid.append(f"{match.group(1)} шт.")
...
# ==========================================
# ОПТИМИЗАЦИЯ КАСТОМНЫХ КАРТИНОК
# ==========================================


class ImageOptimizer:
    @staticmethod
    def optimize_local_images(images_dir: str):
        """Сканирует папку images/ и оптимизирует вес картинок без визуальной потери качества"""
            ...
            if filename.lower().endswith(('.png', '.jpg', '.jpeg')):
                ...
                try:
                    with Image.open(fpath) as img:
                        # Ресайз, если картинка больше 1200px (Яндексу достаточно)
                    ...
                    print(f"⚠️ Сбой оптимизации картинки {filename}: {e}")
...
class PatternManager:
    ...
    def get_all_scores(self, url: str, html_content: str, raw_product: RawExtractedProduct) -> list:
        scores = []
        for p_dict in self.patterns:
            try:
                score = p_dict["obj"].match_score(url, html_content, raw_product)
                scores.append((score, p_dict["name"]))
            except: pass
        scores.sort(key=lambda x: x[0], reverse=True)
        return scores

    def get_best_pattern(self, url: str, html_content: str, raw_product: RawExtractedProduct):
        if not self.patterns: return None
        scored_patterns = []
        for p_dict in self.patterns:
            try:
                score = p_dict["obj"].match_score(url, html_content, raw_product)
        ...
        if not scored_patterns: return None
...
class CacheManager:
    ...
    def generate_fingerprint(self, html_content: str) -> tuple[str, str]:
        soup = BeautifulSoup(html_content, 'lxml')
        ...
        fp_str = f"{h1_title}_{price_val}_{available}"
        return hashlib.md5(fp_str.encode('utf-8')).hexdigest(), fp_str
    ...
    def check_cache(self, url: str, html_content: str) -> tuple[bool, Optional[dict]]:
        ...
        current_hash, current_fp_str = self.generate_fingerprint(html_content)
        cached_item = self.cache[url]
        
        if cached_item.get("fingerprint") == current_hash:
            return True, cached_item.get("raw_data")
            
        if "_0_" in current_fp_str or "unknown_item" in current_fp_str:
            return True, cached_item.get("raw_data")
            
        return False, None

    def update_cache(self, url: str, html_content: str, raw_data: dict):
        fp_hash, fp_str = self.generate_fingerprint(html_content)
        self.cache[url] = {
            "fingerprint": fp_hash, 
            "fp_str": fp_str,
            "raw_data": raw_data, 
...
class DiscoveryAgent:
    @staticmethod
    def detect_cms_and_framework(html_content: str) -> str:
        html_lower = html_content.lower()
        if 'tilda' in html_lower or 'tildacdn' in html_lower: return 'Tilda Publishing'
        if 'bitrix' in html_lower or 'bx-core' in html_lower: return '1C-Bitrix'
        if 'wp-content' in html_lower or 'wordpress' in html_lower: return 'WordPress (WooCommerce)'
        if 'opencart' in html_lower or 'route=product' in html_lower: return 'OpenCart'
        if 'insales' in html_lower: return 'InSales'
        if 'shopify' in html_lower: return 'Shopify'
        if 'data-reactroot' in html_lower or '__next' in html_lower: return 'React/Next.js SPA'
        if 'data-v-' in html_lower or '__nuxt' in html_lower: return 'Vue/Nuxt.js SPA'
        return 'Custom / Unknown / Pure HTML'

    @staticmethod
    def analyze_url_structure(url: str) -> str:
        path = urlparse(url).path.lower().strip('/')
        if not path: return 'Главная страница (Home)'
        parts = path.split('/')
        if len(parts) == 1 and any(w in parts[0] for w in ['catalog', 'category', 'shop', 'store']): return 'Корневой каталог'
        if len(parts) > 1 and any(w in parts[0] for w in ['catalog', 'category']): return 'Подкатегория каталога'
        if len(parts) > 1 and any(w in parts[-2] for w in ['product', 'item', 'p']): return 'Карточка товара'
        if any(w in parts[-1] for w in ['html', 'php', 'tproduct']): return 'Карточка товара (статика/tilda)'
        return 'Смешанный тип (Требует сканирования DOM)'
    ...
    def analyze_and_group_links(base_url: str, html_content: str) -> dict:
        ...
        stop_words = ['login', 'cart', 'korzina', 'tel:', 'mailto:', '.jpg', '.png', 'policy', 'consent', 'contacts', 'kontakt', 'pro-o-nas', 'about', 'oplata', 'dostavka', 'rezerv', 'vozvrat', 'otzyvy', 'faq', 'help', 'garantiya', '+7', '8800', 'javascript:', 'whatsapp', 'viber', 'tg://', 'auth', 'register', 'blog', 'lookbook', 'brand', 'opt', 'pokupatelyam']
        ...
        return dict(sorted(result.items(), key=lambda i: (1 if any(x in i[0] for x in ['product', 'item', 'detail', 'catalog/', 'tproduct/']) else 0, len(i[1])), reverse=True))
    ...
    def extract_product_data(url: str, html_content: str, markdown_content: str = "", domain_rules: dict = None) -> RawExtractedProduct:
        ...
        if selectors:
            try:
                ...
                if selectors.get("images"):
                    ...
                    for img in img_els:
                        src = img.get('src') or img.get('data-original') or img.get('data-src') or img.get('data-lazy')
        ...
        if not images:
            for img_container in soup.find_all(['div', 'a', 'img', 'picture'], class_=re.compile(r'img|image|slider|gallery|photo', re.I)):
                src = img_container.get('src') or img_container.get('data-original') or img_container.get('data-src') or img_container.get('data-lazy')
                if not src and img_container.name != 'img':
        ...
        if not desc_usp:
            desc_container = soup.find(['div', 'section', 'p'], class_=re.compile(r'comment|description|descr|detail-text|about', re.I))
...
class DataTransformer:
    ...
    def generate_numeric_id(text: str) -> str:
        return str(int(hashlib.md5(text.encode('utf-8')).hexdigest(), 16))[:8]

    # 💡 ИДЕАЛЬНЫЙ ID: из артикула или короткий хэш
    @staticmethod
    def generate_offer_id(raw: RawExtractedProduct, url: str) -> str:
        if raw.specs.get('Артикул'):
            clean_sku = re.sub(r'[^a-zA-Z0-9_\-]', '', str(raw.specs['Артикул']))
            if len(clean_sku) >= 3: return clean_sku
        return str(int(hashlib.md5(url.encode('utf-8')).hexdigest(), 16))[:8]
    ...
    def transform_multiple(self, raw: RawExtractedProduct, url: str, category_id_map: dict) -> list[TransformedProduct]:
        ...
        cat_id = category_id_map.get(cat_name_clean, self.generate_numeric_id(cat_name_clean)[:8])
        ...
        # 💡 ИНТЕГРАЦИЯ КАСТОМНЫХ ИЗОБРАЖЕНИЙ ПО СТАБИЛЬНОМУ ID
        stable_id = self.generate_offer_id(raw, url)
        final_images = list(raw.images[:5])
        
        local_img_png = os.path.join("images", f"{stable_id}.png")
        local_img_jpg = os.path.join("images", f"{stable_id}.jpg")
        
        if os.path.exists(local_img_png):
            final_images.insert(0, f"images/{stable_id}.png")
        elif os.path.exists(local_img_jpg):
            final_images.insert(0, f"images/{stable_id}.jpg")
        ...
        if custom_titles_enabled and url in custom_titles_map and custom_titles_map[url]:
            for i, custom_name in enumerate(custom_titles_map[url]):
                ...
                offer_id_val = f"{stable_id}_{i}" if len(custom_titles_map[url]) > 1 else stable_id

                results.append(TransformedProduct(
                    offer_id=offer_id_val, url=url, name=v_title, type_prefix=type_prefix, vendor=vendor_clean, model_name=model_str,
                    price=price_str, oldprice=oldprice_str if valid_oldprice > valid_price else "", currency=raw.currency, images=final_images[:5], description=v_desc, sales_notes=final_sales, specs=safe_specs,
            ...
            for i, tmpl in enumerate(spin_templates):
                ...
                offer_id_val = f"{stable_id}_{i}" if len(spin_templates) > 1 else stable_id
            ...
            results.append(TransformedProduct(
                offer_id=stable_id, url=url, name=v_title, type_prefix=type_prefix, vendor=vendor_clean, model_name=model_str,
                price=price_str, oldprice=oldprice_str if valid_oldprice > valid_price else "", currency=raw.currency, images=final_images[:5], description=v_desc, sales_notes=final_sales, specs=safe_specs,
                custom_labels=[self.clean_emojis_and_specials(lbl)[:175] for lbl in raw.custom_labels[:5]], available="true" if raw.available else "false", category_id=cat_id,
                collection_description=raw.collection_description
            ))
    ...
    def build_feed(self, products: list[TransformedProduct], collections: list[CategoryCollection], output_path: str):
        ...
        if self.config.get("feed_mode", "1") in ['1', '2'] and products:
            ...
            for prod in products:
                ...
                # 💡 ИНТЕГРАЦИЯ RAW ССЫЛОК GITHUB ДЛЯ КАСТОМНЫХ КАРТИНОК
                for img in prod.images:
                    if img.startswith("images/"):
                        site_base = self.config.get("github_repo_raw_base", self.config.get("site_url", ""))
                        img_url = f"{site_base.rstrip('/')}/{img}"
                        self._add_element(offer, 'picture', img_url)
                    else:
                        self._add_element(offer, 'picture', img)
                ...
                for prod in products:
                    ...
                    if prod.images:
                        ...
                        if img.startswith("images/"):
                            site_base = self.config.get("github_repo_raw_base", self.config.get("site_url", ""))
                            img_url = f"{site_base.rstrip('/')}/{img}"
                            self._add_element(cel, 'picture', img_url)
                        else:
                            self._add_element(cel, 'picture', img)
...
async def run_github_worker():
    ...
    ImageOptimizer.optimize_local_images("images")
    ...
    async def safe_fetch(crawler_instance, target_url, delay=1.5, retries=3, scroll=False):
        js_code = """
        async function loadMore() {
            let lastHeight = 0;
            let attempts = 0;
            for (let i = 0; i < 15; i++) {
                let btns = Array.from(document.querySelectorAll('button, a, div, span')).filter(el => {
                    let t = el.innerText ? el.innerText.toLowerCase().trim() : '';
                    return t === 'показать еще' || t === 'загрузить еще' || t === 'показать ещё' || t === 'загрузить ещё' || t === 'еще' || t === 'ещё' || t === 'далее' || t === 'show more' || t === 'load more';
                });
                for (let btn of btns) {
                    if (btn.offsetParent !== null) { btn.click(); await new Promise(r => setTimeout(r, 1000)); }
                }
                window.scrollTo(0, document.body.scrollHeight);
                await new Promise(r => setTimeout(r, 1500));
                let newHeight = document.body.scrollHeight;
                if (newHeight === lastHeight && btns.length === 0) {
                    attempts++;
                    if (attempts > 2) break;
                } else {
                    attempts = 0;
        ...
        for attempt in range(retries):
            try:
                result = await crawler_instance.arun(url=target_url, bypass_cache=True, magic=True, js_code=js_code, delay_before_return_html=delay)
                if result and result.html: return result
            except Exception as e:
                error_msg = str(e)
    ...
    async def process_single_product(product_url: str, parent_category_url: str, crawler_instance=None, html_content_or_block: str = "", is_html_block: bool = False):
        async with semaphore:
            try:
                crawled_product_urls.add(product_url)
                if not is_html_block:
                    await asyncio.sleep(2.5)
                    prod_result = await safe_fetch(crawler_instance, product_url, delay=2.0, scroll=False)
                    if not prod_result or not prod_result.html:
                        return
                    html_content_to_use = prod_result.html
                    markdown_content_to_use = prod_result.markdown
                else:
                    html_content_to_use = html_content_or_block
                    markdown_content_to_use = ""
                
                is_cached, cached_raw_data = cache_manager.check_cache(product_url, html_content_to_use)
                url_in_cache = product_url in cache_manager.cache
                
                if is_cached and cached_raw_data:
                    cache_mode = cached_raw_data.get("extraction_mode", "classic")
                    current_mode = "ai" if use_ai else "classic"
                    if cache_mode != current_mode and config.get("overwrite_cache_mismatch", False):
                        is_cached = False
                ...
                if is_cached and cached_raw_data:
                ...
                elif url_in_cache and not use_ai:
                    temp_raw = scraper.extract_product_data(product_url, html_content_to_use, markdown_content_to_use, domain_rules)
                    ...
                    raw_product = scraper.extract_product_data(product_url, html_content_to_use, markdown_content_to_use, domain_rules)
                    raw_product = apply_single_pattern(product_url, html_content_to_use, raw_product, active_pattern)
                    ...
                    if str(raw_product.price_raw) == "0" or not raw_product.price_raw:
                        try:
                            async with aiohttp.ClientSession() as session:
                                async with session.get(product_url, headers={'User-Agent': 'Mozilla/5.0'}, timeout=5) as resp:
                                    if resp.status == 200:
                                        raw_html = await resp.text()
                                        nuxt_match = re.search(r'[\'"]?price['"]?\s*:\s*(\d+(?:\.\d+)?)', raw_html, re.IGNORECASE)
                                        if nuxt_match: raw_product.price_raw = re.sub(r'[^\d]', '', nuxt_match.group(1))
                    ...
                    if not is_html_block: 
                        async with aiohttp.ClientSession() as session:
                            ...
                            for img_url in raw_product.images:
                                if await validate_image_url(img_url, session): valid_images.append(img_url)
                                if len(valid_images) >= 5: break
                            raw_product.images = valid_images

                    cache_manager.update_cache(product_url, html_content_to_use, raw_product.model_dump())
    ...
    async with AsyncWebCrawler() as crawler:
        if direct_urls_mode:
            for u in url_queue:
                await process_single_product(u, u, crawler)
        else:
            while url_queue:
                ...
                result = await safe_fetch(crawler, current_url, delay=1.5, scroll=True)
                if not result or not result.html: continue

                grouped_links = discovery_agent.analyze_and_group_links(current_url, result.html)
                single_page_blocks = discovery_agent.detect_single_page_catalog(result.html, domain_rules)

                if single_page_blocks and not grouped_links:
                    tasks = []
                    for i, block in enumerate(single_page_blocks):
                        pseudo_url = f"{current_url}#item_{i}"
                        tasks.append(process_single_product(pseudo_url, current_url, crawler, str(block), is_html_block=True))
    ...
    for m_url in missing_urls:
        ...
        if raw_data_dict:
            ...
            # 💡 ТОВАР ПРОПАЛ ИЗ НАЛИЧИЯ - ЗАМЕНЯЕМ НА FALSE
            raw_product.available = False
            cache_manager.update_cache(m_url, "", raw_product.model_dump())
